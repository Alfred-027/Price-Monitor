...
# Determinar las rutas absolutas para los directorios de plantillas y estáticos
template_dir = os.path.abspath('templates')
static_dir = os.path.abspath('static')

# Inicializa la aplicación Flask con rutas absolutas para plantillas y archivos estáticos
...
def index():
    ...
    conn = sqlite3.connect('/app/source/prices.db')
    cursor = conn.cursor()
    cursor.execute('SELECT id, category_name FROM categories WHERE parent_category_id IS NULL ORDER BY category_name ASC')
...
def get_subcategories(category_id):
    conn = sqlite3.connect('/app/source/prices.db')
    cursor = conn.cursor()
    cursor.execute('SELECT id, category_name FROM categories WHERE parent_category_id = ? ORDER BY category_name ASC', (category_id,))
...
def category(category_id):
    ...
    category_name = conn.execute('SELECT category_name FROM categories WHERE id = ?', (category_id,)).fetchone()
    if category_name:
    ...
    products = conn.execute('''
        ...
        WHERE p.category_id = ? OR c.parent_category_id = ?
    ''', (category_id, category_id)).fetchall()
    conn.close()
    return render_template('category.html', category_name=category_name, products=products)

# Ruta para obtener los datos de precios para graficar
@app.route('/price_data', methods=['GET'])
def price_data():
    url = request.args.get('url')
    conn = sqlite3.connect('/app/source/prices.db')
    cursor = conn.cursor()
    cursor.execute('SELECT price, date FROM prices WHERE url = ? ORDER BY date', (url,))
